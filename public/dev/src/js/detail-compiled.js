...
var detailPage = {
    ...
    renderAdvantages: function () {
        ...
        $.each(data.adv_titles.split(/={3,}/), function (i, title) {
            ...
            _items += '<div class="adv-item" style="background-image: url(' + bg + ')">' + '<h2>' + title + '</h2>' + (desc[i] != undefined ? '<p>' + desc[i] + '</p>' : '') + '</div>';
        });
    ...
    renderCases: function () {
        ...
        $.each(icons, function (i, src) {
            list += '<div class="case-item">' + '<img src="' + src + '" alt="">' + '<p>' + texts[i] + '</p></div>';
        });
    ...
    renderDoc: function () {
        ...
        if (data.dev_doc && data.dev_doc.length) {
            ...
            doc.find('a').text(textAndLink[0]).attr('href', textAndLink[1]);
        }
    }
